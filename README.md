...
This branch holds the code for modeling multiple IO dies on a single host. Each IO die can maintain a set of ports. This allows for greater parallelisms.
...
The original UB code base attaches ports directly to each node. This is fine when each node maintains only one UbSwitch instance (or equivalently, one IO die), but doing so makes it harder to identify which port belongs to which IO die in the presence of multiple IO dies. Technically speaking, ports should be maintained by the UbSwitch class, not the nodes. However, to avoid too much refactoring I stuck with the old practice. Ports are still aggregated to the node only, and the following operation is used to determine the IO die a certain port belongs to:
...
It is expected that with **IODieCount** set to 1, the current simulator behaves exactly the same as the original UB simulator. Multiple tests (`2nodes_simple, 2nodes_single-tp, 2nodes_multiple-tp`) have been performed and this was indeed verified.
...
You may refer to the provided test case when making your own test cases. Writing test cases where **IODieCount** and **portsNum** are more realistic (8, 16, 32...) could require a decent amount of effort, as `routing_table.csv`, `topology.csv` and `transport_channel.csv` require lots more lines.
