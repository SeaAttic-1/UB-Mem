...
def measure_throughput_no_internal_traffic(show_graph=True, store_path=None):
    # Measure throughput without any background traffic generated intra-node
    # Comment out the SIM_HBM_INTERNAL macro in control-macro.h before calling this function
    latency = []
    throughput = []
...
def calculate_real_throughput():
    # Apply a linear fit to the data generated by measure_throughput_no_internal_traffic()
...
def measure_throughput_nit_with_different_packet_size():
    # Before calling this function, make sure that the SIM_HBM_INTERNAL macro is commented out in control-macro.h
    # Adjust overhead_enabled variable to match whether the INCLUDE_OTHER_OVERHEAD macro is defined in hbm-macro.h
    end_store_length = 7
    throughput = []
...
'''
    ...
    for mpki in range(1, end_mpki+1):
        ...
        debug_log_hd.write("MPKI="+str(mpki * step)+":\n")
        acc_throughput = 0.0

        with open(MPKI_definition_path, 'w') as mpki_file_hd:
            mpki_file_hd.write(macro_content.format(mpki * step))
        ...
        print("----------------------------------")
        print("MPKI = " + str(mpki * step) + " simulated")
        print("----------------------------------")
...
def make_output_graph():
    ...
    x_axis = [i * step for i in range(0, end_mpki+1)]
    y_axis_1 = None
    ...
    ax2.axhline(1, color='r', linestyle='dotted', label="1%")
    ax2.text(0.02, 1.2, "1%", color='red')
    plt.show()
